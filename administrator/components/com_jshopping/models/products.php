...
class JshoppingModelProducts extends JshoppingModelBaseadmin{
    ...
    function save(array $post){
        ...
        if (!$product->store()){
            $this->setError(_JSHOP_ERROR_SAVE_DATABASE."<br>".$product->_error);
            return 0;
        }
    ...
    function deleteList(array $cid, $msg = 1){
        ...
        foreach($cid as $id){
            $this->delete($id);
            if ($msg){
                $app->enqueueMessage(sprintf(_JSHOP_PRODUCT_DELETED, $id), 'message');
            }
            $res[$id] = true;
        }
    ...
    function copyProduct($pid){
        ...
        JDispatcher::getInstance()->trigger('onBeforeStartCopyProduct', array(&$pid, &$tables, &$languages));
