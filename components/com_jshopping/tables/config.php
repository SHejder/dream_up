...
class jshopConfig extends JTable{
    ...
    function updateNextOrderNumber(){
        $db = JFactory::getDBO();
        $query = "update `#__jshopping_config` set next_order_number=next_order_number+1 where id=".intval($this->id);
        $db->setQuery($query);
        $db->query();
    }
...
//		$this->copyrightText = '<span id="mxcpr"><a rel="nofollow" target="_blank" href="https://www.webdesigner-profi.de/">'.$this->getCopyrightText().'</a></span>';
