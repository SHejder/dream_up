<?php
$menu_view = $params->get('view');
if ($menu_view == accordion)
 {
  class jShopCategoriesHelperIce
   {
    function getCategories($order = 'id', $ordering = 'asc', $publish = 0, $params)
     {
      ...
      if (!$orderby)
        $orderby = "ordering";
      $query = "SELECT `" . $lang->get('name') . "` as name,`" . $lang->get('description') . "` as description,`" . $lang->get('short_description') . "` as short_description, category_id, category_publish, ordering, category_image,category_parent_id FROM `#__jshopping_categories` WHERE " . $add_where . " ORDER BY " . $orderby . " " . $ordering;
      $db->setQuery($query);
        ...
        if (!empty($category_id))
         {
          ...
          $query = "SELECT `" . $lang->get('name') . "` as name, category_id, category_parent_id, category_publish FROM `#__jshopping_categories` where category_publish = '1' ORDER BY category_parent_id, ordering";
          $db->setQuery($query);
        ...
        foreach ($children[$id] as $item)
         {
          ...
          if (!empty($children[$item->category_id]))
           {
            $str .= "<li class='parent item" . $leve . $class . "'>";
            $parent = $params->get('parent');
            if ($parent == 1)
             {
              $parent_images = "<span class='parent_image'>&nbsp;</span>";
             }
            else
             {
              $parent_images = "";
            ...
            if (in_array($item->category_id, $cates))
             {
              $str .= "<a onClick='return toggleShow(\"#slave" . $item->category_id . "\" ,this)' href='#' class='open' style='cursor:pointer'>" . ($show_image ? $item->category_image : "") . $item->name . ($showcounter ? " <span class=\"counter\">(" . self::getTotalItem($children, $item->category_id) . ") </span>" : "") . $parent_images;
             }
            else
             {
              $str .= "<a onClick='return toggleShow(\"#slave" . $item->category_id . "\" ,this)' href='#' class='closed' style='cursor:pointer'>" . ($show_image ? $item->category_image : "") . $item->name . ($showcounter ? " <span class=\"counter\">(" . self::getTotalItem($children, $item->category_id) . ") </span>" : "") . $parent_images;
             }
            if (!empty($children[$item->category_id]))
             {
              $str .= "<i></i></a>";
              if (in_array($item->category_id, $cates))
               {
                $str .= "<ul id='slave" . $item->category_id . "'" . "style='display: block'>";
               }
              else
               {
                $str .= "<ul id='slave" . $item->category_id . "'" . "style='display: none'>";
               }
           ...
           {
            $str .= "<li class='item" . $leve . $class . "'>";
            $str .= "<a href='" . $item->category_link . "' >" . ($show_image ? $item->category_image : "") . $item->name . ($showcounter ? " <span class=\"counter\">(" . self::getTotalItem($children, $item->category_id) . ") </span>" : "");
            if (!empty($children[$item->category_id]))
             {
              $str .= "<i></i></a>";
              $str .= "<ul id='slave" . $item->category_id . "'" . "style='display: none'>";
    ...
    function renderThumb($path, $width = 100, $height = 100, $title = '', $isThumb = true)
     {
      ...
      if (!preg_match("/.jpg|.png|.gif/", strtolower($path)))
        return '&nbsp;';
      if ($isThumb)
       {
        ...
        if (file_exists($imagSource))
         {
          ...
          $thumbPath = JPATH_SITE . DS . 'images' . DS . 'mod_e_jshopping_categories' . DS . str_replace('/', DS, $path);
          ...
          $path = JURI::base() . 'images/mod_e_jshopping_categories/' . $path;
         }
       }
    ...
    function makeDir($path)
     {
      ...
      $tmppath = JPATH_SITE . DS . 'images' . DS . 'mod_e_jshopping_categories' . DS;
