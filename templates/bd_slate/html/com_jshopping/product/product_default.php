...
<form name="product" method="post" action="<?php print $this->action?>" enctype="multipart/form-data" autocomplete="off">
    ...
    <h1><b><?php print $this->product->name?><?php if ($this->config->show_product_code){?> <span class="jshop_code_prod">(<?php print _JSHOP_EAN?>: <span id="product_code"><?php print $this->product->getEan();?></span>)</span><?php }?></b></h1>
    
<table width="100%" border="0">
  <tr>
    <td class="jshop_img_description" width="400px">
  ...
  <tr>
    <td class="image_middle" width="400px">
    ...
    <div class="manufacturer_name">
        <b><?php print _JSHOP_MANUFACTURER?></b>: <span><?php print $this->product->manufacturer_info->name?></span>
    </div>
    <?php }?>
	...
	<?php if (is_array($this->product->extra_field)){?>
        ...
        foreach($this->product->extra_field as $extra_field){
            ...
            ?>
            <div class='extra_fields_group'><small><b><?php print $extra_field_group?></b></small></div>
            <?php }?>
            <div><small><b><span class="extra_fields_name"><?php print $extra_field['name'];?></span></b></small><?php if ($extra_field['description']) {?> <span class="extra_fields_description"><?php print $extra_field['description'];?></span><?php } ?>: <span class="extra_fields_value"><?php print $extra_field['value'];?></span></div>
        <?php }?>
        </div>
    <?php }?>
    ...
    <?php if (!$this->hide_buy){?>                         
        <table width="270px" style="float:left" align="left">
        <tr> 
            <td align="left" width="80px">
                <?php print _JSHOP_QUANTITY?>:
            </td>
            <td class="prod_qty_input" align="left" width="60px">
